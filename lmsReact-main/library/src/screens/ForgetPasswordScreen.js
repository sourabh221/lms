...
import { requestOtpFunc, UpdatePassword } from '../actions/LibraryAction'
...
const ForgetPasswordScreen = (props) => {
  ...
  const { response, loading, error } = libraryEmailOtp

  const libraryPassReset = useSelector((store) => store.libraryPassReset)
  const {
    loading: loading1,
    error: error1,
    response: response1,
  ...
  const passwordConfirmFunc = () => {
    ...
    if (flag === 0) {
      setPassSet(true)
      dispatch(UpdatePassword(email, pwd))
    }
  }
